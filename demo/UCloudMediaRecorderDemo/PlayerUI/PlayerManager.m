...
- (void)noti:(NSNotification *)noti
{
    ...
    else if ([noti.name isEqualToString:UCloudPlayerPlaybackDidFinishNotification])
    {
        ...
        {
            NSLog(@"player manager finish reason playback error");
...
- (void)restartPlayer
{
    ...
    [self buildMediaPlayer:_pathString];
    [self.viewContorller.view sendSubviewToBack:self.mediaPlayer.player.view];
    _retryConnectNumber--;
}
...
- (void)selectedDecodeMthod:(DecodeMethod)decodeMethod
{
    self.current = [self.mediaPlayer.player currentPlaybackTime];
